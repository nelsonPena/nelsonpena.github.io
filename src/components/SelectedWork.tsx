...
export function SelectedWork({ text, locale }: { text: Copy; locale: Locale }) {
  return <section id="work" className="work-section"><div className="section container">
    ...
    <div className="work-grid">{portfolio.work.map((work, index) => <article className="work-card" key={work.id}>
      <div className="work-number" aria-hidden="true">0{index + 1}<span>/</span></div><p className="work-category">{work.category[locale]}</p><h3>{work.title[locale]}</h3><p className="work-company">{work.company}</p><p className="work-description">{work.description[locale]}</p>
      {work.appStoreUrl && <a className="work-link" href={work.appStoreUrl} target="_blank" rel="noopener noreferrer" aria-label={`${work.title[locale]} — ${text.viewAppStore}`}>{text.viewAppStore}<Icon name="arrow" /></a>}
